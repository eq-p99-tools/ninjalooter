...
from ninjalooter import config, logger, logparse, logreplay, models, utils
...
class MenuBar(wx.MenuBar):
    def __init__(self, parent, *args, **kwargs):
        ...
        set_log_bitmap = wx.Bitmap(os.path.join(config.PROJECT_DIR, "data", "icons", "gear.png"))
        ...
        self.confirm_exit_mi = wx.MenuItem(file_menu, wx.ID_ANY, "Confirm Exit", kind=wx.ITEM_CHECK)
        ...
        export_excel_mi = wx.MenuItem(file_menu, wx.ID_FLOPPY, "&Export to Excel\tCtrl+E")
        export_bitmap = wx.Bitmap(os.path.join(config.PROJECT_DIR, "data", "icons", "excel.png"))
        ...
        export_dkp_mi = wx.MenuItem(file_menu, wx.ID_CONVERT, "&Export to EQDKPlus\tCtrl+P")
        export_bitmap = wx.Bitmap(os.path.join(config.PROJECT_DIR, "data", "icons", "export.png"))
        ...
        replay_bitmap = wx.Bitmap(os.path.join(config.PROJECT_DIR, "data", "icons", "reload.png"))
        ...
        clear_bitmap = wx.Bitmap(os.path.join(config.PROJECT_DIR, "data", "icons", "clear.png"))
        ...
        show_ignored_mi = wx.MenuItem(file_menu, wx.ID_ANY, "&Show Ignored Items...")
        ...
        for alliance in config.ALLIANCES:
            alliance_item = wx.MenuItem(self.alliance_menu, wx.ID_ANY, alliance, kind=wx.ITEM_RADIO)
        ...
        for channel in config.DROP_CHANNEL_OPTIONS:
            channel_item = wx.MenuItem(drop_chan_menu, wx.ID_ANY, channel, kind=wx.ITEM_CHECK)
        ...
        for channel in config.BID_CHANNEL_OPTIONS:
            channel_item = wx.MenuItem(bid_chan_menu, wx.ID_ANY, channel, kind=wx.ITEM_CHECK)
        ...
        self.restrict_exports_mi = wx.MenuItem(
            bidding_menu, wx.ID_ANY, "&Restrict Export to Alliance", kind=wx.ITEM_CHECK
        )
        ...
        self.tick_before_loot_mi = wx.MenuItem(
            bidding_menu, wx.ID_ANY, "Tick Before Loot (Quake Mode)", kind=wx.ITEM_CHECK
        )
        ...
        self.remember_player_data_mi = wx.MenuItem(
            bidding_menu, wx.ID_ANY, "Use &Cached Player Data", kind=wx.ITEM_CHECK
        )
        bidding_menu.Append(self.remember_player_data_mi)
        self.remember_player_data_mi.Check(config.REMEMBER_PLAYER_DATA)
        self.Bind(wx.EVT_MENU, self.OnRememberPlayerData, self.remember_player_data_mi)
    ...
    def OnLoadState(self, e: wx.MenuEvent):
        ...
        openFileDialog = wx.FileDialog(
            ...
            os.curdir,
            "",
            "NL State File (state_*.json)|state_*.json",
            wx.FD_OPEN,
        )
    ...
    def OnReplayLog(self, e: wx.MenuEvent):
        ...
        time_select_dialog = wx.Dialog(self.GetParent(), title="Select Time Bounds")
    ...
    def OnSetDropChannel(e: wx.MenuEvent):
        selected_channels = [mi.ItemLabel for mi in e.EventObject.MenuItems if mi.IsChecked()]
        config.MATCH_DROP = [config.DROP_CHANNEL_OPTIONS[chan] for chan in selected_channels]
        logparse.reset_matchers()
        config.CONF.set("default", "drop_channels", ",".join(selected_channels))
        config.write()

    @staticmethod
    def OnSetBidChannel(e: wx.MenuEvent):
        selected_channels = [mi.ItemLabel for mi in e.EventObject.MenuItems if mi.IsChecked()]
        config.MATCH_BID = [config.BID_CHANNEL_OPTIONS[chan] for chan in selected_channels]
        logparse.reset_matchers()
    ...
    def OnConfigure(self, e: wx.MenuEvent):
        ...
        if result == wx.ID_OK:
            ...
            self.GetParent().parser_thread = logparse.ParseThread(self.GetParent())
    ...
    def OnRememberPlayerData(self, e: wx.MenuEvent):
        config.REMEMBER_PLAYER_DATA = self.remember_player_data_mi.IsChecked()
