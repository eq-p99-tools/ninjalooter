...
from ninjalooter import config, models, raidgroups
...
class RaidGroupsFrame(wx.Window):
    def __init__(self, parent: wx.Notebook, *args, **kwargs):
        ...
        parent.GetParent().Connect(-1, -1, models.EVT_CALC_RAIDGROUPS, self.OnCalcRaidGroups)
        ...
        self.raidgroups_main_box.Add(self.no_groups_text, flag=wx.TOP | wx.LEFT, border=10)
    ...
    def OnCalcRaidGroups(self, e: models.CalcRaidGroupsEvent):
        ...
        for group in config.RAID_GROUPS.raid.groups:
            ...
            group_type_label = wx.StaticText(self, label="%s Group" % group.group_type)
            group_type_label.SetFont(self.label_font)
            group_box.Add(group_type_label)
            group_list = ObjectListView.ObjectListView(
                self, wx.ID_ANY, size=wx.Size(250, 142), style=wx.LC_REPORT | wx.LC_SINGLE_SEL
            )
            ...
            self.raidgroups_main_box.Add(group_box, flag=wx.TOP | wx.LEFT, border=10)
