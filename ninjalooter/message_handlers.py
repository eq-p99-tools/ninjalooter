...
from ninjalooter import config, extra_data, logger, models, utils
...
def handle_raidtick(match: re.Match, skip_store=False) -> bool:
    tick_time = match.group("time")
    config.LAST_RAIDTICK = dateutil.parser.parse(tick_time)
    return True


def handle_creditt(match: re.Match, skip_store=False) -> bool:
    ...
    signals.creditt.emit()
    return True


def handle_gratss(match: re.Match, skip_store=False) -> bool:
    time = match.group("time")
    user = match.group("from")
    message = match.group("message")
    raw_message = match.group(0)
    ...
    if award_match:
        for old_auction in config.HISTORICAL_AUCTIONS.values():
            ...
            if (
                old_auction.highest()
                and old_auction.highest()[0]
                == (
                    award_match.group("name"),
                    int(award_match.group("dkp")),
                )
                and old_auction.item.name == award_match.group("item")
            ):
                return False
...
def handle_start_who(match: re.Match, skip_store=False) -> bool:
    config.LAST_WHO_SNAPSHOT.clear()
    signals.clear_who.emit()
    return True
...
def raidtick_reminder_alert() -> None:
    try:
        reminder_message = (
            ...
            "You will be reminded %d more times." % (config.MAX_RAIDTICK_REMINDERS - config.RAIDTICK_REMINDER_COUNT)
        )
        if config.RAIDTICK_REMINDER_COUNT >= config.MAX_RAIDTICK_REMINDERS:
        ...
        if config.RAIDTICK_REMINDER_COUNT < config.MAX_RAIDTICK_REMINDERS:
            ...
            config.RAIDTICK_ALERT_TIMER = threading.Timer(10 * 60, raidtick_reminder_alert)
            config.RAIDTICK_ALERT_TIMER.start()
    except Exception:
        LOG.exception("Error in raidtick reminder alert callback")


def handle_end_who(match: re.Match, skip_store=False) -> bool:
    ...
    if raidtick_was <= datetime.timedelta(seconds=3):
        ...
        config.RAIDTICK_ALERT_TIMER = threading.Timer(60 * 60, raidtick_reminder_alert)
        config.RAIDTICK_ALERT_TIMER.start()
    ...
    signals.who_end.emit()
    if not skip_store:
        utils.store_state()
    return True


def handle_who(match: re.Match, skip_store=False) -> bool:
    ...
    if name not in config.PLAYER_DB:
        ...
        LOG.info(
            "Updating player history for %s from %s to %s",
            name,
        ...
        LOG.info(
            "Updating player history for RP %s from %s to %s",
            name,
            config.PLAYER_DB[name].guild,
            guild,
        )
    ...
    if name == config.PLAYER_NAME and guild:
        ...
        if alliance:
            ...
            config.write()
            signals.alliance_changed.emit(alliance)
    ...
    try:
        ...
        level_int = 0
    signals.who.emit(name, pclass, level_int, guild or "")
    return True


def handle_drop(match: re.Match, skip_store=False) -> list:
    ...
    for item in found_items:
        if item.lower() in utils.get_active_item_names():
            LOG.debug("Skipping drop %s because it is already up for auction.", item)
            continue
        for pending in config.PENDING_AUCTIONS:
            ...
            if item.lower() == pending.name.lower() and elapsed < config.DROP_COOLDOWN:
                skip = True
                LOG.debug("Skipping drop %s because of DROP_COOLDOWN config.", item)
                break
        if skip:
            skip = False
            continue
        ...
        if (
            config.NODROP_ONLY
            and item in extra_data.EXTRA_ITEM_DATA
...
def handle_bid(match: re.Match, skip_store=False) -> bool:
    name = match.group("name")
    if name == "You":
    ...
    found_items = utils.get_items_from_text(text)
    if not found_items:
        LOG.info("%s might have attempted to bid but no item name found: %s", name, text)
        return False
    if len(found_items) > 1:
        LOG.info("%s attempted to bid for two items at once: %s", name, found_items)
        return False
    item = found_items[0]
    ...
    for auc_item in config.ACTIVE_AUCTIONS.values():
        if item.lower() == auc_item.name().lower():
            if not isinstance(auc_item, models.DKPAuction):
                LOG.info("Ignoring bid by %s because `%s` is a random auction.", name, item)
                return False
            if config.RESTRICT_BIDS and guild and alliance != auc_item.alliance:
                LOG.info(
                    "%s attempted to bid for %s, but is in the wrong guild/alliance: %s/%s",
                    name,
                    item,
                    guild,
                    alliance,
            ...
            signals.bid.emit(auc_item)
            # pylint: disable=protected-access
            if (
                config.SECOND_MAIN_REMINDER_DKP
...
def handle_auc_start(match: re.Match, skip_store=False) -> bool:
    ...
    if "bid" in match.groupdict() and match.group("bid") is not None:
        ...
        if item_name.lower() in active_items:
            LOG.debug("Not starting auction for %s, it is still in-progress.", item_name)
            return False
        ...
        for item in config.HISTORICAL_AUCTIONS.values():
            ...
            if item.name().lower() == item_name.lower() and highest == item_bidder:
                historical_auc = item
                break
        if historical_auc:
            LOG.debug(
                "Found historical auction for %s/%s/%s, restarting it.",
                item_name,
                item_bidder[0][0],
                item_bidder[0][1],
            )
            ...
            return True
        LOG.debug(
            "Failed to find historical auction for %s/%s/%s.",
            item_name,
            item_bidder[0][0],
            item_bidder[0][1],
        )
    ...
    if not pending_item:
        LOG.debug("Tried to start auction for %s but one was not pending.", item_name)
        return False
    ...
    if not auc:
        LOG.warning("Failed to start auction for %s, old auction pending.", item_name)
        return False
    if number:
        auc.number = number
...
def handle_auc_end(match: re.Match, skip_store=False) -> bool:
...
def handle_rand1(match: re.Match, skip_store=False) -> str:
    return match.group("name")


def handle_rand2(match: re.Match, skip_store=False) -> bool:
    name = match.group("name")
    ...
    for item_obj in config.ACTIVE_AUCTIONS.values():
        if isinstance(item_obj, models.RandomAuction) and item_obj.number == rand_to:
            if rand_from > 0:
                LOG.info("%s rolled from %d instead of 0, not counting it.", name, rand_from)
                return False
            item_obj.add(rand_result, name)
            signals.bid.emit(item_obj)
            if not skip_store:
                utils.store_state()
            return True
